...
// // A single cubie piece. It receives its state (position and rotation) from the main Cube component.
...
//             currentMove.current.progress = Math.min(1, currentMove.current.progress + 0.1); // Animation speed

//             if (currentMove.current.progress >= 1) {
//                 // Animation for one move is complete
//                 isAnimating.current = false;
//                 // Apply the final state transformation to prevent any drift
...
//             // Check if this cubie is part of the moving face
...
import { Canvas, useFrame } from '@react-three/fiber';
import { Edges, OrbitControls } from '@react-three/drei'; // Import Edges
import * as THREE from 'three';

// A single cubie piece. It now includes an <Edges> component for the black borders.
const Cubie = ({ position, rotation }) => {
    const meshRef = useRef();
    ...
    return (
        <mesh ref={meshRef}>
            ...
            {/* This adds the black edges to the cubie */}
...
const Cube = ({ animationSequence }) => {
    ...
    useFrame(() => {
        if (isAnimating.current && currentMove.current) {
            // --- SPEED CHANGE IS HERE ---
            // Changed from 0.1 to 0.05 to make the animation slower
